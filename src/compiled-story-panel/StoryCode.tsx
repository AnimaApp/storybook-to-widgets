...
const StoryCode = () => {
  ...
  const storiesInfoMapped = Object.fromEntries(Object.entries(storiesInfo).map(([key, story]) => {
    return [
      story.id, {
        ...story,
        story_id: story.id,
  ...
  const rootStoryInfo = Object.values(storiesInfoMapped).find((story) => story.root);
